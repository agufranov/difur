...
/** пузырь между стенкой и антистенкой: плато полуширины w с крутыми краями */
const bubble = (s: number, w: number) => 0.5*(Math.tanh((s+w)*7) - Math.tanh((s-w)*7));
...
const SCEN_ART: Record<string, { f: (s: number) => number; gh?: (s: number) => number;
  ...
  /* φ⁴: у всех трёх сценариев начальные данные ОДНИ И ТЕ ЖЕ — стенка и антистенка
     на ±12, различается только скорость сближения. Поэтому сплошная кривая у них
     одна (пузырь) и стрелки одни (стенки идут навстречу), а различает картинки
     пунктир «станет таким»: схлопнется, ужмётся или разъедется. Это единственные
     сценарии, где по начальным данным их не отличить вовсе, — если однажды
     ghost отсюда уберут, все три значка станут одинаковыми. */
  trap:   { f:  s => bubble(s, 0.55), gh: s => bubble(s, 0.07), ar:[[-0.45, 1], [0.45, -1]] },
  bound:  { f:  s => bubble(s, 0.55), gh: s => bubble(s, 0.24), ar:[[-0.45, 1], [0.45, -1]] },
  free:   { f:  s => bubble(s, 0.55), gh: s => bubble(s, 0.92), ar:[[-0.45, 1], [0.45, -1]] }
};
